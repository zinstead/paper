...
import { useStructureDesignStore } from "@/store/molstar";
...
export class Sequence<P extends SequenceProps> extends PluginUIComponent<P> {
  ...
  // 选中crop区域上的某些残基
  state = {
    cropTarget: useStructureDesignStore.getState().cropTarget,
    specifyHotspot: useStructureDesignStore.getState().specifyHotspot,
    seqIdxToResidueMap: new Map<number, ResidueItem>(),
  };

  unsubscribeCropAndHotspot?: () => void;
  ...
  componentDidMount() {
    ...
    this.unsubscribeCropAndHotspot = useStructureDesignStore.subscribe(
      (state) => {
        this.setState((prev) => ({
          ...prev,
          cropTarget: state.cropTarget,
          specifyHotspot: state.specifyHotspot,
        }));
      }
    );
    ...
    const seqIdxToResidueMap = new Map<number, ResidueItem>();
    for (let i = 0, il = sw.length; i < il; ++i) {
      const residueId = Number.parseInt(this.getSequenceNumber(i));
      const unit = this.getLoci(i)?.elements[0].unit;
      if (unit) {
        const chainId = unit.model.atomicHierarchy.chains.label_asym_id.value(
          unit.chainGroupId
        );
        seqIdxToResidueMap.set(i, { chainId, residueId });
      }
    }
    this.setState((prev) => ({ ...prev, seqIdxToResidueMap }));
  }
  ...
  componentWillUnmount() {
    ...
    this.unsubscribeCropAndHotspot?.();
  }
  ...
  protected getBackgroundColor(seqIdx: number) {
    ...
    const residue = this.state.seqIdxToResidueMap.get(seqIdx);
    ...
    if (
      residue &&
      this.state.specifyHotspot[residue.chainId]?.includes(residue.residueId)
    ) {
      return this.markerColors.highlighted;
    }
    if (
      residue &&
      this.state.cropTarget[residue.chainId]?.includes(residue.residueId)
    ) {
      return this.markerColors.croped;
    }
